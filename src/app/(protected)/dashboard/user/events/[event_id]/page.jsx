...
const EventDetailsPage = () => {
  ...
  useEffect(() => {
    if (event) {
      // Use event_slug if available, fallback to generated slug from name
      const identifier = event.event_slug || generateEventSlug(event.name || event.event_name);
  ...
  const handleCopyLink = () => {
    // Use event_slug if available, fallback to generated slug from name
    const identifier = event?.event_slug || generateEventSlug(event?.name || event?.event_name || "") || slug;
  ...
  }, [ticketSelections, event, checkoutPaymentMethod, appliedCoupon]);
