 ...
 */
export default function BulkBookForAttendeeModal({
  isOpen,
  onClose,
  event,
  eventId,
  onSuccess,
  onManualPaymentRequired,
}) {
  ...
  const defaultCategoryName =
    categories.length > 0 ? categories[0].name : "General Admission";
  ...
  const categoryOptions = categories.map((cat) => ({
    value: cat.name,
    label: isPaidEvent
      ? `${cat.name} - ₦${cat.price?.toLocaleString() || 0}`
      : cat.name,
  }));
  ...
  const bulkTotalAmount = useMemo(() => {
    ...
    return attendees.reduce((sum, a) => {
      const cat = categories.find(
        (c) => c.name === (a.category_name || defaultCategoryName),
      );
  ...
  const parseExcel = (arrayBuffer) => {
    ...
    const getColIdx = () => {
      ...
      const first = rawHeader.findIndex(
        (h) => /first|given|fname/i.test(h) || h === "firstname",
      );
      const last = rawHeader.findIndex(
        (h) => /last|surname|lname|family/i.test(h) || h === "lastname",
      );
      const email = rawHeader.findIndex(
        (h) => h.includes("email") || h === "e-mail",
      );
      const cat = rawHeader.findIndex(
        (h) => h.includes("category") || h === "ticket",
      );
      return {
    ...
    for (let i = startRow; i < rows.length; i++) {
      ...
      const category_name =
        String(row[idx.category] ?? "").trim() || defaultCategoryName;
  ...
  const progressPercent =
    ticketCount > 0 ? Math.round((validCount / ticketCount) * 100) : 0;
  ...
  const handleSubmit = async () => {
    ...
    if (invalidIndices.length > 0) {
      if (invalidIndices.length <= 5) {
        toast.error(
          `Please complete attendee forms: #${invalidIndices.join(", #")}`,
        );
      } else {
    ...
    try {
      ...
      const response = await api.post(
        "/tickets/organizer/bulk-book-for-attendees/",
        payload,
      );
      ...
      <div className="relative bg-linear-to-b from-[#111111] to-[#0A0A0A] border border-white/10 rounded-2xl sm:rounded-3xl w-full max-w-2xl shadow-2xl max-h-[95vh] sm:max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="relative px-4 sm:px-6 pt-4 sm:pt-6 pb-3 sm:pb-4 shrink-0">
              ...
              <div>
                ...
                <p className="text-[10px] sm:text-[11px] text-gray-500">
                  {step === 1
                    ...
                    : `Fill details for ${ticketCount} attendee(s)`}
                </p>
              </div>
            </div>
            <button
          ...
          <div className="flex items-center gap-2 mt-4">
            <div
              className={`flex-1 h-1 rounded-full transition-colors ${step >= 1 ? "bg-emerald-500" : "bg-white/10"}`}
            />
            <div
              className={`flex-1 h-1 rounded-full transition-colors ${step >= 2 ? "bg-emerald-500" : "bg-white/10"}`}
            />
          </div>
        </div>
                    ...
                    <tbody className="text-gray-400">
                      <tr>
                        <td className="py-1 px-2">
                          A (or header: First name / firstname)
                        </td>
                        <td className="py-1 px-2">First name</td>
                        <td className="py-1 px-2">Yes</td>
                      </tr>
                      <tr>
                        <td className="py-1 px-2">
                          B (or header: Last name / lastname)
                        </td>
                        <td className="py-1 px-2">Last name</td>
                        <td className="py-1 px-2">Yes</td>
                      </tr>
                      <tr>
                        <td className="py-1 px-2">
                          C (or header: Email / email)
                        </td>
                        <td className="py-1 px-2">Email address</td>
                        <td className="py-1 px-2">Yes</td>
                      </tr>
                      <tr>
                        <td className="py-1 px-2">
                          D (or header: Category / ticket)
                        </td>
                        <td className="py-1 px-2">Ticket category name</td>
                        <td className="py-1 px-2">Optional</td>
                      </tr>
                    </tbody>
                  </table>
                  <p>
                    Row 1 can be a header row (e.g. First name, Last name,
                    Email, Category). Data starts from row 2. CSV: same order,
                ...
                {attendees.map((attendee, idx) => (
                  <AttendeeForm
                    ...
                    onUpdate={(field, value) =>
                      updateAttendee(idx, field, value)
                    }
